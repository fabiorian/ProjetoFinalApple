...
class DonationsHouse: Identifiable, Decodable {
    ...
    init(name: String, category: Category, description: String, latitude: Double, longitude: Double) {
        self.id = UUID()
        self.name = name
    ...
    var coordenadas: CLLocation {   CLLocation(latitude: latitude, longitude: longitude)   }
...
func donationHouses() -> [DonationsHouse] {
    ...
    let house4 = DonationsHouse(
        name: "Centro Robocop",
        category: .eletronic,
        description: "Coletamos eletrônicos, no geral.",
        latitude: -3.750000,
